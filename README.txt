...
每个归档目录都会生成便于人工查看的 `archive_log.txt`，简要列出结果、工程路径、资源统计、导出内容和失败项；同时生成供程序校验的 `package_manifest.json`。只有 JSON 状态为 `success`、失败列表为空、源工程匹配且归档 HIP 确实存在时才算打包完成；JSON 缺失、hython 异常退出或复制类临时故障会自动重试，单个 HIP 最多尝试 3 次。重试会复用同一个归档目录并保留已复制资源；资源不存在等确定性错误会直接报告，不再整轮重跑。每次尝试前都会移除旧清单，无法移除时不会启动打包或接受旧结果。
